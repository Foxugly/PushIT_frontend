 ...
 *
 *     Failed to fetch dynamically imported module: .../chunk-6EYZE7OG.js
 *
 * Seen in production on poker (POKER-FRONTEND-4) and tm (TM-FRONTEND-5); the
 * same deploy pattern applies here.
...
export class StaleChunkService {
  ...
  private static readonly RELOAD_MARKER = 'pushit.staleChunkReloadAt';
  ...
  private offerReload(): void {
    this.messages.add({
      ...
      sticky: true,
      closable: true,
      summary: this.transloco.translate('app.update.title'),
      detail: this.transloco.translate('app.update.detail'),
    });
  }
