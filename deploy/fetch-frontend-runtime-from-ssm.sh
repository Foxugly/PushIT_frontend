...
# CSP report-uri, dérivé du Sentry DSN pour reporter les violations à Sentry.
# DSN https://<key>@<host>/<project> -> endpoint Security Header
# https://<host>/api/<project>/security/?sentry_key=<key>. Émis comme le fragment
# nginx "; report-uri <endpoint>" ajouté en fin de header CSP ; vide (pas de DSN
# / DSN malformé) -> la CSP ne porte pas de report-uri.
csp_report = ""
...
if dsn:
    ...
    if key and host and project:
        endpoint = f"{u.scheme}://{host}{port}/api/{project}/security/?sentry_key={key}"
        csp_report = f"; report-uri {endpoint}"
...
lines = [
    ...
    f'set $pushit_turnstile_site_key  "{values["TURNSTILE_SITE_KEY"]}";',
    ...
    f'set $pushit_csp_report          "{csp_report}";',
]
